	...
	public	function fillObject($dr, $obj) {
		try {
			...
			if (!method_exists($this, $method)) {
				throw new Exception('No existe el m�todo ' . $method . ' en la clase "Mapper".');
			}
...
*/
	...
	private function fillColorPorArticulo($dr, ColorPorArticulo $colorPorArticulo) {
		try {
			...
			$colorPorArticulo->fotos[11] = array_key_exists('zoom_cana', $dr) ? $dr['zoom_cana'] : (array_key_exists('zoom_ca�a', $dr) ? $dr['zoom_ca�a'] : null);
			...
			$colorPorArticulo->textoCania = array_key_exists('texto_cana', $dr) ? $dr['texto_cana'] : (array_key_exists('texto_ca�a', $dr) ? $dr['texto_ca�a'] : null);
...
$colorPorArticulo->referenciaWebMayorista = $dr['referencia_web_mayorista'];
	...
	private function fillPatron($dr, Patron $patron) {
		try {
			...
			$patron->disenio = Funciones::keyIsSet($dr, 'diseno') ? Funciones::keyIsSet($dr, 'diseno') : Funciones::keyIsSet($dr, 'dise�o');
	...
	private function fillPedidoItem($dr, PedidoItem $pedidoItem) {
		try {
			...
			$pedidoItem->precioUnitario = $dr['precio_unitario'];
				for ($i = 1; $i <= 10; $i++) {
					if ($dr['anulado'] == 'N') {
						$pedidoItem->cantidad[$i] = Funciones::keyIsSet($dr, 'cant_' . $i, 0);
						$pedidoItem->pendiente[$i] = Funciones::keyIsSet($dr, 'pend_' . $i, 0);
						$pedidoItem->predespachados[$i] = Funciones::keyIsSet($dr, 'pred_' . $i, 0);
						$pedidoItem->tickeados[$i] = Funciones::keyIsSet($dr, 'tick_' . $i, 0);
					} else {
	...
	private function mapperQueryAcreditarDebitarChequeCabecera(AcreditarDebitarChequeCabecera $acreditarDebitarChequeCabecera, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($acreditarDebitarChequeCabecera->fecha) . '), ';
		...
		try {
			if ($modo == Modos::select){
				...
				*/
				...
				//Cuando se modifica un art�culo, tambi�n pongo la fecha de �ltima modificaci�n en sus colores (para sincronizar luego con ecommerce)
	...
	private function mapperQueryAsientoContable(AsientoContable $asientoContable, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($asientoContable->fecha) . '), ';
				...
				$sql .= 'fecha_asiento = dbo.toDate(' . Datos::objectToDB($asientoContable->fecha) . '), ';
	...
	private function mapperQueryCierrePeriodoFiscal(CierrePeriodoFiscal $cierrePeriodoFiscal, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($cierrePeriodoFiscal->fechaDesde) . '), ';
				$sql .= 'dbo.toDate(' . Datos::objectToDB($cierrePeriodoFiscal->fechaHasta) . '), ';
				$sql .= Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= 'GETDATE() ';
				$sql .= '); ';
				...
				$sql .= 'fecha_desde = dbo.toDate(' . Datos::objectToDB($cierrePeriodoFiscal->fechaDesde) . '), ';
				$sql .= 'fecha_hasta = dbo.toDate(' . Datos::objectToDB($cierrePeriodoFiscal->fechaHasta) . '), ';
				$sql .= 'cod_usuario_ultima_mod = ' . Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= 'fecha_ultima_mod = GETDATE() ';
				$sql .= 'WHERE cod_cierre_periodo = ' . Datos::objectToDB($cierrePeriodoFiscal->id) . '; ';
			} elseif ($modo == Modos::delete) {
	...
	private function mapperQueryCheque(Cheque $cheque, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= 'dbo.toDate(' . Datos::objectToDB($cheque->fechaEmision) . '), ';
				$sql .= 'dbo.toDate(' . Datos::objectToDB($cheque->fechaVencimiento) . '), ';
				$sql .= 'GETDATE()';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
				...
				$sql .= 'fecha_credito_debito = dbo.toDate(' . Datos::objectToDB($cheque->fechaCreditoDebito) . '), ';
				$sql .= 'fecha_vencimiento = dbo.toDate(' . Datos::objectToDB($cheque->fechaVencimiento) . '), ';
				$sql .= 'fecha_emision= dbo.toDate(' . Datos::objectToDB($cheque->fechaEmision) . '), ';
				$sql .= 'cod_usuario_ultima_mod = ' . Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= 'fecha_ultima_mod = ' . 'GETDATE()' . ' ' ;
				$sql .= 'WHERE cod_cheque = ' . Datos::objectToDB($cheque->id) . '; ';
			} elseif ($modo == Modos::delete) {
	...
	private function mapperQueryChequera(Chequera $chequera, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($chequera->fecha) . '), ';
	...
	private function mapperQueryCobroChequeVentanillaCabecera(CobroChequeVentanillaCabecera $cobroChequeVentanillaCabecera, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($cobroChequeVentanillaCabecera->fecha) . '), ';
				$sql .= 'GETDATE()';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
	...
	private function mapperQueryCobroChequeVentanillaTemporal(CobroChequeVentanillaTemporal $cobroChequeVentanillaTemporal, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($cobroChequeVentanillaTemporal->fecha) . '), ';
	...
	private function mapperQueryColorPorArticulo(ColorPorArticulo $colorPorArticulo, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'zoom_ca�a = ' . Datos::objectToDB($colorPorArticulo->fotos[11]) . ', ';
    ...
    private function mapperQueryDepositoBancarioCabecera(DepositoBancarioCabecera $depositoChequeCabecera, $modo){
        ...
        try {
            if ($modo == Modos::select){
                ...
                $sql .= Datos::objectToDB(Usuario::logueado()->id) . ', ';
                $sql .= 'dbo.toDate(' . Datos::objectToDB($depositoChequeCabecera->fecha) . '), ';
                $sql .= 'GETDATE()';
                $sql .= '); ';
            } elseif ($modo == Modos::update) {
    ...
    private function mapperQueryDepositoBancarioTemporal(DepositoBancarioTemporal $depositoBancarioTemporal, $modo){
        ...
        try {
            if ($modo == Modos::select){
                ...
                $sql .= 'dbo.toDate(' . Datos::objectToDB($depositoBancarioTemporal->fecha) . '), ';
                ...
                $sql .= 'fecha = dbo.toDate(' . Datos::objectToDB($depositoBancarioTemporal->fecha) . '), ';
			...
			//} elseif ($modo == Modos::update) {
				...
				foreach ($despacho->detalle as $despachoItem) {
					...
					if ($despachoItem->remitido()) {
						...
						throw new FactoryException('No se puede borrar el despacho ya que uno de sus items pertenece a un remito. Debe borrar el remito n�mero ' . $despachoItem->remitoNumero . ' primero');
					}
	...
	private function mapperQueryDespachoItem(DespachoItem $despachoItem, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				//Es necesario controlar que no est� anulado para que no sigan aument�ndose los predespachados
				//La otra opcion es bajar las cantidades del despacho_d a 0
				if ($despachoItem->anulado == 'S')
					throw new FactoryException('No se puede borrar el despacho porque ya est� anulado');
	...
	private function mapperQueryDocumentoProveedor(DocumentoProveedor $documentoProveedor, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($documentoProveedor->fecha) . '), ';
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($documentoProveedor->fechaVencimiento) . '), ';
				$sql .= 'dbo.toDate(' . Datos::objectToDB($documentoProveedor->fechaPeriodoFiscal) . '), ';
				...
				$sql .= 'fecha = dbo.toDate(' . Datos::objectToDB($documentoProveedor->fecha) . '), ';
				...
				$sql .= 'fecha_vencimiento = dbo.toDate(' . Datos::objectToDB($documentoProveedor->fechaVencimiento) . '), ';
				$sql .= 'fecha_periodo_fiscal = dbo.toDate(' . Datos::objectToDB($documentoProveedor->fechaPeriodoFiscal) . '), ';
	...
	private function mapperQueryDocumentoProveedorAplicacion(DocumentoProveedorAplicacion $documentoProveedorAplicacion, $modo, $primeraVez = true){
		...
		try {
			if ($modo == Modos::select) {
				...
				// if ($documentoProveedorAplicacion->esHaber() && $documentoProveedorAplicacion->tipoDocumento) { // Si no buscamos tambi�n por tipo_docum, puede ocurrir que haya m�s de un registro que cumple con los mismos requerimientos (me pas� que hab�a una FAC y una OP)
                //     $sql .= 'AND tipo_docum = ' . Datos::objectToDB($documentoProveedorAplicacion->tipoDocumento) . ' '; // Entonces eso hac�a que detecte como que no existe un �nico registro, y fallaba
                // }
				$sql .= '; ';
			//} elseif ($modo == Modos::insert) {
			} elseif ($modo == Modos::update) {
				...
				if ($primeraVez) {
					...
					switch ($documentoProveedorAplicacionHaber->tipoDocumento) {
						...
						case 'OP':
							...
							break;
						default:
							throw new Exception('No se reconoce el tipo de documento al intentar aplicar. Deber�a ser "OP" o "REN"'); break;
					}
				}
			//} elseif ($modo == Modos::delete) {
			} else {
	...
	private function mapperQueryEcommerce_OrderDetail(Ecommerce_OrderDetail $ecommerce_OrderDetail, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= '); ';
			//} elseif ($modo == Modos::update) {
			} elseif ($modo == Modos::delete) { //Tal vez esto deber�a ser DELETE
	...
	private function mapperQueryEjercicioContable(EjercicioContable $ejercicioContable, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($ejercicioContable->fechaDesde) . '), ';
				$sql .= 'dbo.toDate(' . Datos::objectToDB($ejercicioContable->fechaHasta) . '), ';
				$sql .= Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= 'GETDATE()';
				$sql .= '); ';
				...
				$sql .= 'fecha_desde = dbo.toDate(' . Datos::objectToDB($ejercicioContable->fechaDesde) . '), ';
				$sql .= 'fecha_hasta = dbo.toDate(' . Datos::objectToDB($ejercicioContable->fechaHasta) . '), ';
				$sql .= 'cod_usuario_ultima_mod = ' . Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= 'fecha_ultima_mod = ' . 'GETDATE()' . ' ' ;
				$sql .= 'WHERE cod_ejercicio_contable = ' . Datos::objectToDB($ejercicioContable->id) . '; ';
			} elseif ($modo == Modos::delete) {
		...
		try {
			if ($modo == Modos::select){
				...
				if (!$factura->tieneDetalle()) {
					//Si tieneDetalle == 'S' es porque no tiene articulos en despachos_d, sino en documentos_d. No es el camino com�n
				...
				//Si no tiene CAE, puedo volver to_do atr�s! Si tiene CAE, no
				if (isset($factura->cae))
					throw new FactoryException('No se puede borrar la factura ya que tiene CAE. Debe hacer una nota de cr�dito');
				if (!$factura->tieneDetalle()) {
					foreach ($factura->detalle as $remito) {
						$sql .= 'UPDATE remitos_c SET ';
	...
	private function mapperQueryFichaje(Fichaje $fichaje, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'ubicacion_tipo ';			//D�nde fich� entrada
				//$sql .= 'ubicacion_confirmada ';	//D�nde fich� salida
				$sql .= ') VALUES (';
    ...
    private function mapperQueryForecast(Forecast $forecast, $modo){
        ...
        try {
            if ($modo == Modos::select){
                ...
                $sql .= 'GETDATE(), ';
                $sql .= 'dbo.toDate(' . Datos::objectToDB($forecast->fechaInicio) . '), ';
                $sql .= 'dbo.toDate(' . Datos::objectToDB($forecast->fechaFin) . '), ';
                ...
                $sql .= 'Fecha = dbo.toDate(' . Datos::objectToDB($forecast->fecha) . '), ';
                $sql .= 'Fecha_ingreso = dbo.toDate(' . Datos::objectToDB($forecast->fechaInicio) . '), ';
                $sql .= 'Fecha_Fin = dbo.toDate(' . Datos::objectToDB($forecast->fechaFin) . '), ';
	...
	private function mapperQueryGastito(Gastito $gastito, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($gastito->fecha) . '), ';
				...
				$sql .= 'fecha = dbo.toDate(' . Datos::objectToDB($gastito->fecha) . '), ';
	...
	private function mapperQueryKoiTicket(KoiTicket $koiTicket, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= Datos::objectToDB($koiTicket->usuario->id) . ', '; //Est� as� para los casos en los que viene de un ticket delegado
				$sql .= 'GETDATE() ';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
	...
	private function mapperQueryHorma(Horma $horma, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dise�ador = ' . Datos::objectToDB($horma->diseniador) . ', ';
		...
		try {
			if ($modo == Modos::select){
				...
				if (isset($notaDeCredito->cae))
					throw new FactoryException('No se puede borrar la nota de cr�dito ya que tiene CAE. Debe hacer una nota de d�bito');
				$sql .= 'UPDATE documentos_c SET ';
				$sql .= 'cod_usuario_baja = ' . Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= 'anulado = ' . Datos::objectToDB('S') . ', ';
				$sql .= 'fecha_baja = GETDATE() ';
		...
		try {
			if ($modo == Modos::select){
				...
				if (isset($notaDeDebito->cae))
					throw new FactoryException('No se puede borrar la nota de d�bito ya que tiene CAE. Debe hacer una nota de cr�dito');
				$sql .= 'UPDATE documentos_c SET ';
				$sql .= 'cod_usuario_baja = ' . Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= 'anulado = ' . Datos::objectToDB('S') . ', ';
				$sql .= 'fecha_baja = GETDATE() ';
	...
	private function mapperQueryOrdenDePago(OrdenDePago $ordenDePago, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= 'dbo.toDate(' . Datos::objectToDB($ordenDePago->fecha) . '), ';
				$sql .= 'GETDATE() ';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
				$sql .= 'UPDATE orden_de_pago SET ';
	...
	private function mapperQueryPatron(Patron $patron, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dise�o = ' . Datos::objectToDB($patron->disenio) . ', ';
	...
	private function mapperQueryPrestamo(Prestamo $prestamo, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= is_null($prestamo->fecha) ? 'GETDATE(), ' : 'dbo.toDate(' . Datos::objectToDB($prestamo->fecha) . '), ';
				$sql .= 'GETDATE() ';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
	...
	private function mapperQueryProveedorMateriaPrima(ProveedorMateriaPrima $proveedorMateriaPrima, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($proveedorMateriaPrima->fecha) . '), ';
				$sql .= Datos::objectToDB($proveedorMateriaPrima->codigoInterno) . ', ';
				$sql .= Datos::objectToDB($proveedorMateriaPrima->anulado) . ', ';
				$sql .= 'dbo.toDate(' . Datos::objectToDB($proveedorMateriaPrima->fechaBaja) . ') ';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
	...
	private function mapperQueryRecibo(Recibo $recibo, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= is_null($recibo->fecha) ? 'GETDATE(), ' : 'dbo.toDate(' . Datos::objectToDB($recibo->fecha) . '), ';
				$sql .= 'GETDATE() ';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
				$sql .= 'UPDATE recibo SET ';
	...
	private function mapperQueryRendicionGastos(RendicionGastos $rendicionGastos, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= 'dbo.toDate(' . Datos::objectToDB($rendicionGastos->fecha) . '), ';
				$sql .= 'GETDATE() ';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
				$sql .= 'UPDATE rendicion_de_gastos SET ';
	...
	private function mapperQueryRetencionEfectuada(RetencionEfectuada $retencionEfectuada, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($retencionEfectuada->fecha) . '), ';
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= 'GETDATE() ';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
				...
				$sql .= 'fecha = dbo.toDate(' . Datos::objectToDB($retencionEfectuada->fecha) . '), ';
				$sql .= 'fecha_ultima_mod = GETDATE() ';
				$sql .= 'WHERE cod_retencion = ' . Datos::objectToDB($retencionEfectuada->id) . '; ';
			} elseif ($modo == Modos::delete) {
				$sql .= 'UPDATE retencion_efectuada SET ';
	...
	private function mapperQueryRetencionSufrida(RetencionSufrida $retencionSufrida, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($retencionSufrida->fecha) . '), ';
				$sql .= Datos::objectToDB('N') . ', ';
				$sql .= 'GETDATE() ';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
				...
				$sql .= 'fecha = dbo.toDate(' . Datos::objectToDB($retencionSufrida->fecha) . '), ';
				$sql .= 'fecha_ultima_mod = GETDATE() ';
				$sql .= 'WHERE cod_retencion = ' . Datos::objectToDB($retencionSufrida->id) . '; ';
			} elseif ($modo == Modos::delete) {
				$sql .= 'UPDATE retencion_sufrida SET ';
	...
	private function mapperQueryRol(Rol $rol, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				//Le borro las funcionalidades pq despu�s se van a guardar en el transaction
	...
	private function mapperQueryRolPorUsuario(RolPorUsuario $rolPorUsuario, $modo){
		...
		try {
			if ($modo == Modos::select){
				//Le pido los campos uno por uno para as� ponerles ALIAS y cuando hago
				//alg�n getListObject, no me tire problemas en el WHERE (puedo no poner la tabla en el where)
	...
	private function mapperQueryStock(Stock $stock, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'AND cod_articulo = ' . Datos::objectToDB($stock->idArticulo) . ' ';
				$sql .= 'AND cod_color_articulo = ' . Datos::objectToDB($stock->idColorPorArticulo) . '; ';
				//Cuando se modifica un stock, tambi�n pongo la fecha de �ltima modificaci�n en sus colores (para sincronizar luego con ecommerce)
				$sql .= 'UPDATE colores_por_articulo SET ';
				$sql .= 'fechaUltimaMod = GETDATE() ';
	...
	private function mapperQueryTransferenciaInternaCabecera(TransferenciaInternaCabecera $transferenciaInternaCabecera, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($transferenciaInternaCabecera->fechaDocumento) . '), ';
	...
	private function mapperQueryUsuario(Usuario $usuario, $modo){
		...
		try {
			...
			//} elseif ($modo == Modos::insert) { Se hace INSERT s�lo de UsuarioLogin
			} elseif ($modo == Modos::update) {
				$sql .= 'DELETE FROM roles_por_usuario ';
				$sql .= 'WHERE cod_usuario = ' . Datos::objectToDB($usuario->id) . '; ';
				$sql .= 'UPDATE users SET ';
	...
	private function mapperQueryVentaChequesCabecera(VentaChequesCabecera $ventaChequesCabecera, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= Datos::objectToDB(Usuario::logueado()->id) . ', ';
				$sql .= 'dbo.toDate(' . Datos::objectToDB($ventaChequesCabecera->fecha) . '), ';
				$sql .= 'GETDATE()';
				$sql .= '); ';
			} elseif ($modo == Modos::update) {
	...
	private function mapperQueryVentaChequesTemporal(VentaChequesTemporal $ventaChequesTemporal, $modo){
		...
		try {
			if ($modo == Modos::select){
				...
				$sql .= 'dbo.toDate(' . Datos::objectToDB($ventaChequesTemporal->fecha) . '), ';
