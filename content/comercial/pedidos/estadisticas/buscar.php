	...
	switch ($modo) {
		case 1: //ENCABEZADO: Vendedor || DETALLE: Cliente
			return array(
				...
				'detalleItem' => '[' . $item->idCliente . '] ' . $item->cliente->razonSocial . '<label class="fRight pRight5">Calificaci�n: ' . $item->cliente->calificacion . '</label>'
			);
		case 2: //ENCABEZADO: || DETALLE: Art�culo
			return array(
				'idC1' => 0,
				'idC2' => $item->idAlmacen . '_' . $item->idArticulo . '_' . $item->idColorPorArticulo,
				'titulo' => '',
				'tituloDetalle' => 'Art�culo',
				'detalleItem' => '[' . $item->getIdCombinado('-') . '] ' . $item->articulo->nombre . ' ' . $item->colorPorArticulo->nombre
			);
		case 3: //ENCABEZADO: Cliente || DETALLE: Art�culo
			return array(
				'idC1' => $item->idCliente,
				'idC2' => $item->idAlmacen . '_' . $item->idArticulo . '_' . $item->idColorPorArticulo,
		...
		case 4: //ENCABEZADO: Art�culo || DETALLE: Cliente
		default:
			return array(
...
function meterEnArray(&$array, $arrIdC, $item) {
	...
	if (!isset($array[$arrIdC['idC1']]['detalle'][$arrIdC['idC2']])) {
		...
		//La cantidad devuelta de ese art�culo debe setearse una �nica vez. El resto se va sumando
