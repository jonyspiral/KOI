...
<script>
    ...
    Koi.controller('CatalogoCtrl', function ($scope, $filter, ServiceCliente, ServiceCatalogo) {
        ...
        $scope.getImageUrls = function (articulo) {
            // console.log('Artículo:', articulo); // Depuración
            // console.log('Imágenes disponibles:', articulo.imagenesArticulo); // Depuración
            let images = [];
            if (articulo.imagenesArticulo) {
                images = articulo.imagenesArticulo.map(imagen => ({
                    ruta: $scope.imagesUrl + imagen.ruta,
                    orden: imagen.orden || 0 // Usa 0 como valor predeterminado si no hay orden
                }));
                // console.log('URLs generadas con orden:', images); // Depuración
            }
        ...
        let changeStarFavorite = function (element, typeChange) {
            if (!typeChange) {
                element.querySelector('i').classList.remove('star-on');
                element.querySelector('i').classList.add('star-off');
            } else {
                element.querySelector('i').classList.remove('star-off');
                element.querySelector('i').classList.add('star-on');
            }
        };
        ...
        $scope.getIdNameModal3d = function (articulo) {
            return '#modal' + articulo.idArticulo + articulo.idColorPorArticulo;
        };

        $scope.getNameModal3d = function (articulo) {
            return 'modal' + articulo.idArticulo + articulo.idColorPorArticulo;
        };

        $scope.getArticuloCodigoColor = function (articulo) {
            return articulo.idArticulo + '-' + articulo.idColorPorArticulo;
        };

        $scope.getArticuloRutaIframe3D = function (articulo) {
            return articulo.rutaIframe3D;
        };

        $scope.getIdStarArticulo = function(articulo) {
            return "star-" + articulo.idArticulo + '-' + articulo.idColorPorArticulo;
        };
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-2 col-md-2 col-lg-2 text-left skip-row-top">
            ...
            <div class="descripcion_lateral">
              <?php echo $familiaDescripcion; ?></div>
        </div>
