...
/* log */
$__log = __DIR__ . '/../tmp/borrar.debug.log';
...
__dbg('== borrar.php START ==', array('sid'=>function_exists('session_id')?session_id():null));
...
/* body: aceptar JSON o x-www-form-urlencoded legado */
$raw = @file_get_contents('php://input'); $decoded = json_decode($raw, true);
$idArt = isset($decoded['idArticulo']) ? $decoded['idArticulo'] : (isset($_POST['idArticulo']) ? $_POST['idArticulo'] : '');
$idCol = isset($decoded['idColorPorArticulo']) ? $decoded['idColorPorArticulo'] :
         (isset($decoded['idColor']) ? $decoded['idColor'] :
          (isset($_POST['idColorPorArticulo']) ? $_POST['idColorPorArticulo'] :
           (isset($_POST['idColor']) ? $_POST['idColor'] : '')));

if ($idArt === '' || $idCol === '') { echo json_encode(array('status'=>400,'message'=>'Faltan claves','data'=>array())); exit; }
...
$sql = "DELETE FROM favoritos_cliente
        WHERE cod_cliente = '".addslashes($clienteId)."'
          AND cod_articulo = '".addslashes($idArt)."'
          AND cod_color_articulo = '".addslashes($idCol)."'; ";
__dbg('DELETE.sql', $sql);
try {
  Datos::EjecutarSQLsinQuery($sql);
  echo json_encode(array('status'=>200,'message'=>'success','data'=>array(array('idArticulo'=>$idArt,'idColorPorArticulo'=>$idCol,'deleted'=>1))));
} catch (Exception $ex) {
  __dbg('DELETE.err', $ex->getMessage());
