  ...
  Koi.controller('FavoritosCtrl', function ($scope, ServiceCliente) {
    ...
    $scope.toggleFavorito2 = function (articulo, $event) {
      ...
      prom.then(function (res) {
        ...
        if (!nuevoEstado) {
          angular.forEach($scope.favoritos, function (linea, idLinea) {
            angular.forEach(linea.items, function (item, key) {
              if (item === articulo) {
                delete linea.items[key];
              }
            });

            if (!linea.items || Object.keys(linea.items).length === 0) {
              delete $scope.favoritos[idLinea];
            }
          });
  ...
  <div ng-if="show(favoritos)">
    ...
    <div class="row well">
      <div class="col-xs-12">
        ...
        <div style="margin-top: 15px;">
          <button type="button" class="btn btn-generar-pedido" ng-click="showSucursales = true"
            ng-show="!showSucursales">
          ...
          <div class="form-group" ng-show="showSucursales">
            ...
            <select class="form-control" id="sucursal" ng-model="idSucursalPedido"
              style="max-width: 320px; margin: 0 auto;">
              <option ng-repeat="sucursal in sucursales" value="{{ sucursal.id }}">{{ sucursal.id }} - {{
                sucursal.nombre }}</option>
            </select>
          </div>

          <button type="button" class="btn btn-generar-pedido btn-success" ng-click="confirmarPedido(idSucursalPedido)"
            ng-show="showSucursales">
            <i class="fa fa-fw fa-shopping-cart"></i> Confirmar pedido
          </button>
          <button type="button" class="btn btn-generar-pedido" ng-click="showSucursales = false"
            ng-show="showSucursales">
    ...
    <div class="row" ng-repeat="(idLinea, linea) in favoritos">
      <div class="col-xs-12">
        <div class="row total">
          <div class="col-xs-12">
            <h2><b>[{{ linea.nombre }}]</b> {{ sumTotalParesPorLinea(idLinea) }} pares - {{
              funciones.formatearMoneda(sumTotalCostoPorLinea(idLinea)) }}</h2>
          </div>
        </div>
        ...
        <div class="row favorito"
          ...
          <div class="col-sm-3 item-imagen">
            <div class="item-inner">
              ...
              <div class="item-star">
                <a ng-attr-id="{{getIdStarArticulo(articulo.fav)}}" href=""
              ...
              <div class="item-precios">
                <span class="subarticulo sub-inline-grid" ng-repeat="subArticulo in articulo.subArticulos">
                  <span>{{ subArticulo.nombre + ' - ' + subArticulo.idArticulo + ' ' + subArticulo.idColorPorArticulo
                    }}</span>
                  <span>
                    <span data-toggle="popover" data-html="true" data-placement="bottom" tabindex="0"
                      item-stock="{{subArticulo.cantidadTalles}}">
                ...
                <span class="subarticulo" ng-repeat="subarticuloi in articulo.subArticulos">
                  ...
                  <span>
                    ...
                    <span class="badge badge-danger"
                      ng-if="subarticuloi.colorPorArticulo.tipoProductoStock.descuentoPorc">-{{
                      subarticuloi.colorPorArticulo.tipoProductoStock.descuentoPorc }}%</span>
                  </span>
            ...
            <div ng-repeat="subArticulo in articulo.subArticulos">
              <table class="tabla-curvas">
                ...
                <tbody>
                  ...
                  <!-- Libre -->
                  <tr class="row-curva"
                    ng-if="subArticulo.formaDeComercializacion == 'L' || subArticulo.formaDeComercializacion == 'T'">
                    ...
                    <td ng-repeat="(i, talle) in subArticulo.talles">
                      <input maxlength="3" type="number" step="1" min="0" max="999"
                        ng-model="subArticulo.paresLibres[i]" ng-change="updateLibre(subArticulo, i, talle)" />
                    </td>
                  </tr>

                  <!-- Modular -->
                  <tr class="row-curva" ng-if="subArticulo.formaDeComercializacion == 'M'"
                    ng-repeat="(j, curva) in subArticulo.curvas">
                    <td>Curva {{ j+1 }}</td>
                    ...
                    <td>
                      <button type="button" class="btn" ng-click="addCurva(subArticulo, curva)"><i
                          class="fa fa-fw fa-plus"></i></button>
                      <button type="button" class="btn" ng-disabled="curva.unidadesSeleccionadas <= 0"
                        ng-click="removeCurva(subArticulo, curva)"><i class="fa fa-fw fa-minus"></i></button>
                    </td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr class="row-totales">
                    ...
                    <td colspan="{{ subArticulo.formaDeComercializacion == 'M' ? '2' : '1' }}"
                      style="background: #eaeaea;"></td>
                  </tr>
                </tfoot>
              </table>
            </div>
          ...
          <div class="col-sm-2 col-totales">
            <table>
              <tbody>
                ...
                <tr>
                  <td class="titulo no-border">$ Total</td>
                  <td class="valor no-border total">{{ funciones.formatearMoneda(sumTotalArticuloCosto(articulo)) }}
                  </td>
                </tr>
              </tbody>
            </table>
