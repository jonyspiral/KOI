...
class UserController extends Controller
{
    public function index(Request $request)
    {
        ...
        $carpeta_vistas = $config['carpeta_vistas'] ?? 'produccion/abms/users';
       ...
       $registros = $query->paginate($perPage);
        ...
        {
            $configPath = resource_path("meta_abms/config_form_User.json");
            $config = File::exists($configPath) ? json_decode(File::get($configPath), true) : [];
            $camposRaw = $config['campos'] ?? [];
            $campos = array_filter($camposRaw, fn($cfg) => !empty($cfg['incluir']));
            $modelo = 'User';
            ...
            foreach ($campos as $campo => $meta) {
                ...
                if (($meta['input_type'] ?? null) === 'autonumerico') {
                    $siguiente[$campo] = User::max($campo) + 1;
                }
            ...
            return view('sistemas/users.create', compact(
        ...
        public function edit($id)
        {
            ...
            $registro = User::where($primaryKey, $id)->firstOrFail();
            ...
            return view('sistemas/users.edit', compact(
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('sistemas.abms.user.index')->with('success', '✅ Registro creado y sincronizado.');
    }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        return $this->redirectToParent($request->merge($registro->toArray()), $modeloNombre)
            ?? redirect()->route('sistemas.abms.user.index')->with('success', $mensaje);
    }
    ...
    public function show($id)
    {
        ...
        return view('sistemas/users.show', compact('registro', 'campos'));
    }
...
public function restaurar($id)
{
    ...
    return redirect()->route('sistemas.abms.user.index')->with('success', 'Registro restaurado correctamente.');
}
}
