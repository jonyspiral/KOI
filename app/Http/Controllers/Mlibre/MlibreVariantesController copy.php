    ...
    public function sincronizarSeleccionados(Request $request)
{
    ...
    foreach ($ids as $id) {
        ...
        // ✅ Guardamos SIEMPRE el nuevo SCF (si es diferente y no vacío)
        ...
        // ✅ Guardamos SIEMPRE antes de intentar sincronizar
        $v->save();

        // Validación SCF (solo para sincronizar)
...
public function sincronizarFiltrados(Request $request)
{
    ...
    $query = MlVariante::query()->with('skuVariante', 'publicacion');
    ...
    if ($variantes->isEmpty()) {
        return back()->with('error', 'No hay variantes para sincronizar con los filtros aplicados.');
    }
    ...
    foreach ($variantes as $v) {
        try {
            ...
            $v->manual_price = $v->manual_price ?? 0;
            $v->manual_stock = $v->manual_stock ?? 0;

            if (!$v->manual_price && $v->precio != $v->skuVariante->precio) {
                $v->precio = $v->skuVariante->precio;
            }

            if (!$v->manual_stock && $v->stock != $v->skuVariante->stock) {
                $v->stock = $v->skuVariante->stock;
            }
