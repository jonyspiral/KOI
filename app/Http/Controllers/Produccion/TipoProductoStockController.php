...
class TipoProductoStockController extends Controller
{
    public function index(Request $request)
    {
        // 🧩 Cargar configuración JSON del formulario
        $modelo = 'TipoProductoStock';
        ...
        $carpeta_vistas = $config['carpeta_vistas'] ?? 'produccion/abms/tipo_producto_stocks';
       ...
       $registros = $query->paginate($perPage);
        ...
        {
            $configPath = resource_path("meta_abms/config_form_TipoProductoStock.json");
            $config = File::exists($configPath) ? json_decode(File::get($configPath), true) : [];
            $camposRaw = $config['campos'] ?? [];
            $campos = array_filter($camposRaw, fn($cfg) => !empty($cfg['incluir']));
            $modelo = 'TipoProductoStock';
            ...
            foreach ($campos as $campo => $meta) {
                ...
                if (($meta['input_type'] ?? null) === 'autonumerico') {
                    $siguiente[$campo] = TipoProductoStock::max($campo) + 1;
                }
            ...
            return view('produccion/abms/tipo_producto_stock.create', compact(
        ...
        public function edit($id)
        {
            ...
            $registro = TipoProductoStock::where($primaryKey, $id)->firstOrFail();
            ...
            return view('produccion/abms/tipo_producto_stock.edit', compact(
    ...
    {
        $modeloNombre = 'TipoProductoStock';
        ...
        return redirect()->route('produccion.abms.tipo_producto_stock.index')->with('success', '✅ Registro creado y sincronizado.');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $modeloSql = "\\App\\Models\\Sql\\{$modeloNombre}";
        $rutaNombre = 'produccion.abms.tipo_producto_stock';
    ...
    public function destroy(Request $request, $id)
    {
        ...
        return $this->redirectToParent($request->merge($registro->toArray()), $modeloNombre)
            ?? redirect()->route('produccion.abms.tipo_producto_stock.index')->with('success', $mensaje);
    }
    ...
    public function show($id)
    {
        ...
        return view('produccion/abms/tipo_producto_stock.show', compact('registro', 'campos'));
    }
...
public function restaurar($id)
{
    ...
    return redirect()->route('produccion.abms.tipo_producto_stock.index')->with('success', 'Registro restaurado correctamente.');
}
}
