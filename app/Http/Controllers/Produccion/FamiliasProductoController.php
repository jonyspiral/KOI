    ...
    public function index(Request $request)
{
    ...
    $carpeta_vistas = $config['carpeta_vistas'] ?? 'produccion/abms/familias_productos';

    $query = FamiliasProducto::query();
...
public function create()
{
    ...
    return view('produccion/abms/familias_producto.create', compact(
    ...
    public function edit($id)
    {
        ...
        $registro = FamiliasProducto::where($primaryKey, $id)->firstOrFail();
        ...
        return view('produccion/abms/familias_producto.edit', compact(
    ...
    {
        $modeloNombre = 'FamiliasProducto'; // ← Nombre del modelo (se reemplaza dinámicamente)
        ...
        return redirect()->route('produccion.abms.familias_producto.index')->with('success', '✅ Registro creado y sincronizado.');
    }
...
{
    $modeloNombre = 'FamiliasProducto';
    $modeloClase = "\\App\\Models\\{$modeloNombre}";
    $rutaNombre = 'produccion.abms.familias_producto';
...
public function destroy(Request $request, $id)
{
    ...
    if ($config['sincronizable'] ?? false) {
        ...
        if (!$primaryKeySql || !isset($registro->{$primaryKeySql})) {
            ...
            $conexionSql = (new $modeloSql)->getConnectionName();
            $ok = $syncService->syncDelete('FamiliasProducto', $registro->toArray(), $primaryKeySql, $conexionSql);
    ...
    return $this->redirectToParent($request->merge($registro->toArray()), 'FamiliasProducto')
        ?? redirect()->route('produccion.abms.familias_producto.index')->with('success', $mensaje);
}
   ...
   // 📦 Redirección automática al padre (si es subformulario)
    ...
    public function show($id)
    {
        ...
        return view('produccion/abms/familias_producto.show', compact('registro', 'campos'));
    }
    ...
    public function restaurar($id)
{
    ...
    return redirect()->route('produccion.abms.familias_producto.index')->with('success', 'Registro restaurado correctamente.');
}
