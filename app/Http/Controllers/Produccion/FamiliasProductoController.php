   ...
   // $perPage = max(10, min($perPage, 500));
    ...
    public function create()
    {
        ...
        foreach ($campos as $campo => $meta) {
            ...
            if (($meta['input_type'] ?? null) === 'autonumerico') {
                $siguiente[$campo] = FamiliasProducto::max($campo) + 1;
            }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($campos as $campo => $meta) {
            ...
            if (($meta['input_type'] ?? null) === 'autonumerico') {
                $valor = FamiliasProducto::max($campo) + 1;
        ...
        $redirect = $this->redirectToParent($request, 'FamiliasProducto');
        return $redirect ?? redirect()->route('produccion.abms.familias_producto.index')->with('success', 'Guardado correctamente.');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $redirect = $this->redirectToParent($request, 'FamiliasProducto');
        return $redirect ?? redirect()->route('produccion.abms.familias_producto.index')->with('success', 'Actualizado correctamente.');
    }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $registro = FamiliasProducto::where('id', $id)->firstOrFail();
        ...
        return $this->redirectToParent($request->merge($registro->toArray()), $modeloNombre)
            ?? redirect()->route('produccion.abms.familias_producto.index')->with('success', 'Registro eliminado correctamente.');
    }
