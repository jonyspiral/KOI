    ...
    public function index(Request $request)
{
    ...
    $carpeta_vistas = $config['carpeta_vistas'] ?? 'produccion/abms/almacens';
   ...
   // $perPage = max(10, min($perPage, 500));
    ...
    {
        $configPath = resource_path("meta_abms/config_form_Almacen.json");
        $config = File::exists($configPath) ? json_decode(File::get($configPath), true) : [];
        $camposRaw = $config['campos'] ?? [];
        $campos = array_filter($camposRaw, fn($cfg) => !empty($cfg['incluir']));
        $modelo = 'Almacen';
        ...
        foreach ($campos as $campo => $meta) {
            ...
            if (($meta['input_type'] ?? null) === 'autonumerico') {
                $siguiente[$campo] = Almacen::max($campo) + 1;
            }
        ...
        return view('produccion/abms/almacenes.create', compact(
    ...
    public function edit($id)
    {
        ...
        $registro = Almacen::where($primaryKey, $id)->firstOrFail();
        ...
        return view('produccion/abms/almacenes.edit', compact(
...
try {
    ...
    $sincronizador->syncCreate('Almacen', $datos, 'desarrollo'); // Usamos 'desarrollo' como destino de SQL Server
} catch (\Exception $e) {
    ...
    return redirect()->route('produccion.abms.almacenes.index')->withErrors('Error en la sincronización con SQL Server');
}

        $redirect = $this->redirectToParent($request, 'Almacen');
        return $redirect ?? redirect()->route('produccion.abms.almacenes.index')->with('success', 'Guardado correctamente.');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $redirect = $this->redirectToParent($request, 'Almacen');
        return $redirect ?? redirect()->route('produccion.abms.almacenes.index')->with('success', 'Actualizado correctamente.');
    }
    ...
    public function destroy(Request $request, $id)
{
    ...
    return $this->redirectToParent($request->merge($registro->toArray()), 'Almacen')
        ?? redirect()->route('produccion.abms.almacenes.index')->with('success', 'Marcado como eliminado correctamente.');
}
    ...
    public function show($id)
    {
        ...
        return view('produccion/abms/almacenes.show', compact('registro', 'campos'));
    }
    /**
