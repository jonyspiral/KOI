...
public function destroy(Request $request, $id)
{
    ...
    if ($foreignKey && $request->has($foreignKey)) {
        $registro = PasosRutasProduccion::where($foreignKey, $request->input($foreignKey))->firstOrFail();
    } else {
        $registro = PasosRutasProduccion::where($primaryKey, $id)->firstOrFail();
    }
    ...
    return $this->redirectToParent($request->merge($registro->toArray()), $modeloNombre)
        ?? redirect()->route('produccion.abms.pasos_rutas_produccion.index')->with('success', 'Registro eliminado correctamente.');
}
