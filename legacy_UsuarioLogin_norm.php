...
// aqui comenzo el codigo de SOFIA
/* 			$sessionId = session_id();
			$sql = "IF EXISTS (SELECT * FROM koi_sessions WHERE session_id = :sid)
					BEGIN
						UPDATE koi_sessions 
						SET user_id = :uid,
							updated_at = GETDATE()
			...
			$stmt->execute([
				'sid' => $sessionId,
				'uid' => $usuarioLogueado->id, // o cod_usuario
			]);
 */// aqui termiana SOFIA
			...
			//Va a entrar por acß si el usuario no existe (username) o si la contrase±a es incorrecta
			self::logout();
