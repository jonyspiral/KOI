...
(function (a, b) {
	...
	function h(a) {
		var b = g[a] = {},
			c, d;
		a = a.split(/\s+/);
		for (c = 0, d = a.length; c < d; c++) b[ac] = !0;
		return b
	}
	var c = a.document,
		d = a.navigator,
			...
			e.fn = e.prototype = {
				...
				init: function (a, d, f) {
					...
					if (typeof a == "string") {
						...
						if (g && (g[1] || !d)) {
							if (g[1]) {
								d = d instanceof e ? d[0] : d, k = d ? d.ownerDocument || d : c, j = n.exec(a), j ? e.isPlainObject(d) ? (a = [c.createElement(j[1])], e.fn.attr.call(a, d, !0)) : a = [k.createElement(j[1])] : (j = e.buildFragment([g1], [k]), a = (j.cacheable ? e.clone(j.fragment) : j.fragment).childNodes);
	...
	f.extend({
			...
			if ( !! f.acceptData(a)) {
				...
				if (b) {
					...
					if (d) {
						...
						for (e = 0, g = b.length; e < g; e++) delete d[be];
		...
		data: function (a, c) {
			...
			if (c === b) {
				h = this.triggerHandler("getData" + d[1] + "!", [d0]), h === b && this.length && (h = f.data(this[0], a), h = l(this[0], a, h));
	...
	function (a, b, c) {
		...
		var m = function (b, d, e, f) {
				...
				if (w.length > 1 && p.exec(b)) if (w.length === 2 && o.relative[w0]) j = y(w[0] + w[1], d, f);
				else {
					j = o.relative[w0] ? [d] : m(w.shift(), d);
	...
	f.fn.extend({
		...
		filter: function (a, b, c) {
			c && (a = ":not(" + a + ")");
			return b.length === 1 ? f.find.matchesSelector(b[0], a) ? [b0] : [] : f.find.matches(a, b)
		},
		dir: function (a, c, d) {
			var e = [],
				g = a[c];
	...
	bY = bT.exec(bX.toLowerCase()) || [], f.fn.extend({
		...
		ajax: function (a, c) {
			...
			}(d.data && d.hasContent && d.contentType !== !1 || c.contentType) && v.setRequestHeader("Content-Type", d.contentType), d.ifModified && (k = k || d.url, f.lastModified[k] && v.setRequestHeader("If-Modified-Since", f.lastModified[k]), f.etag[k] && v.setRequestHeader("If-None-Match", f.etag[k])), v.setRequestHeader("Accept", d.dataTypes[0] && d.accepts[d.dataTypes0] ? d.accepts[d.dataTypes0] + (d.dataTypes[0] !== "*" ? ", " + bZ + "; q=0.01" : "") : d.accepts["*"]);
