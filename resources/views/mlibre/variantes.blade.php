...
<div class="container-fluid">
    ...
    <form method="GET" action="{{ route('mlibre.variantes.index') }}" class="row g-2 mb-3">
        ...
        {{-- Filtro rápido: posibles para sync stock --}}
        <div class="col-md-3">
            <label class="form-label fw-light text-muted small mb-1">Posibles para sync stock</label>
            <select name="filtro_sync_stock" class="form-select" onchange="this.form.submit()">
                <option value="">-- Todos --</option>
                <option value="1" {{ request('filtro_sync_stock') == '1' ? 'selected' : '' }}>Solo candidatos</option>
            </select>
        </div>

        {{-- Filtros por otros campos --}}
    ...
    {{-- FORMULARIO DE SINCRONIZACIÓN SELECCIONADOS --}}
    <form method="POST" action="{{ route('mlibre.sync.seleccionados') }}">
        @csrf
            ...
            <tbody>
                @forelse($variantes as $v)
                    <tr>
                        ...
                        <td @if(isset($v->skuVariante) && $v->skuVariante->ml_price != $v->precio) class="table-warning" @endif>
                            {{ $v->skuVariante->ml_price ?? '-' }}
                        </td>
    ...
    <form method="POST" action="{{ route('mlibre.sync.pendientes') }}">
        @csrf
        <button type="submit" class="btn btn-secondary">🕒 Sincronizar pendientes</button>
    </form>
