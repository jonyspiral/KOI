...
<div class="col-md-2 d-flex align-items-end">
    <div class="form-check">
        <input class="form-check-input me-2" type="checkbox" name="diferencia_stock" id="diferencia_stock"
               value="1" {{ request('diferencia_stock') ? 'checked' : '' }}
               onchange="this.form.submit()">
        <label class="form-check-label" for="diferencia_stock">
            🔍 Diferencia Stock
        </label>
    </div>
</div>
        ...
        @foreach(['ml_id', 'variation_id', 'seller_custom_field', 'color', 'talle', 'modelo', 'titulo', 'status', 'family_id', 'logistic_type'] as $campo)
            <div class="col-md-3">
                <label class="form-label small text-muted">{{ ucfirst(str_replace('_', ' ', $campo)) }}</label>
        ...
        <div class="form-check col-md-2 d-flex align-items-end">
            <input class="form-check-input me-2" type="checkbox" name="has_campaign" id="has_campaign"
        ...
        <div class="col-md-2 d-flex align-items-end">
            <a href="{{ route('mlibre.variantes.index', ['reset' => 1]) }}" class="btn btn-danger w-100">
                ❌ Borrar filtros
            </a>
        </div>
    </form>
    ...
    <form method="POST" action="{{ route('mlibre.sync.seleccionados') }}" id="sync-form">
        ...
        @if(session('sync_result'))
            <div class="alert alert-info">
                ...
                Total: {{ session('sync_result.total') }}<br>
                ✅ OK: {{ session('sync_result.ok') }}<br>
            ...
            <tbody>
                @forelse($variantes as $v)
                    <tr>
                        ...
                        @php
                            $stockSku = optional($v->skuVariante)->stock ?? null;
                            $hayDiferencia = $stockSku !== null && $stockSku != $v->stock;
                        @endphp

                        <td class="{{ $hayDiferencia ? 'table-danger' : 'table-success' }}">
                            {{ $stockSku ?? '—' }}
                        </td>
                        ...
                        <td title="{{ $v->sync_log_stock }}">{{ $v->sync_status_stock ?? '🕓' }}</td>
                        <td title="{{ $v->sync_log_precio }}">{{ $v->sync_status_precio ?? '🕓' }}</td>
                    </tr>
                @empty
                    <tr>
        ...
        <button type="submit" class="btn btn-warning mt-2">🔄 Sincronizar seleccionados</button>
    </form>
