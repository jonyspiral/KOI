...
<div class="container-fluid px-0">
    ...
    @else
        <a href="{{ route('produccion.abms.articulos_new.create') }}" class="btn btn-success mb-3">➕ Nuevo</a>
    @endif

    <form action="{{ route('produccion.abms.articulos_new.index') }}" method="GET" class="mb-3 d-flex flex-wrap gap-2">
        <div class="input-group">
            <input type="text" name="buscar" value="{{ request('buscar') }}" class="form-control" placeholder="Buscar...">
            <button type="submit" class="btn btn-outline-primary">Buscar</button>
            @if(request('buscar'))
                <a href="{{ route('produccion.abms.articulos_new.index') }}" class="btn btn-outline-secondary">Limpiar</a>
            @endif
        </div>
    ...
    <tr x-data="{ showSubform: false }" class="{{ $eliminado ? 'fila-eliminada' : '' }}">
        ...
        <td class="text-end">
            <div class="d-flex gap-1 justify-content-end">
                @if (!$eliminado)
                    <a href="{{ route('produccion.abms.articulos_new.edit', $registro[$primaryKey]) }}" class="btn btn-sm btn-primary">✏️</a>
                    <form action="{{ route('produccion.abms.articulos_new.destroy', $registro[$primaryKey]) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar este registro?')">
                        @csrf
                        @method('DELETE')
                ...
                @if ($eliminado)
                    <form action="{{ route('produccion.abms.articulos_new.restaurar', $registro->{$primaryKey}) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PUT')
...
@endforeach
    ...
    @if ($formViewType === 'modal')
        @include('produccion/abms/articulos_new.create-modal', ['registro' => []])
    @endif
</div>
@endsection
