...
<div class="container-fluid px-0">
    ...
    <form action="{{ route('produccion.abms.articulos.index') }}" method="GET" class="mb-3 d-flex flex-wrap gap-2">
        ...
        <td class="text-end">
            <div class="d-flex gap-1 justify-content-end">
                ...
                <button 
                    ...
                    <form action="{{ route('produccion.abms.articulos.destroy', $registro[$primaryKey]) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar este registro?')">
                        @csrf
                        @method('DELETE')
                ...
                @if ($eliminado)
                <form action="{{ route('produccion.abms.articulos.restaurar', $registro->{$primaryKey}) }}" method="POST" style="display:inline">
                    @csrf
...
@endphp
    @if ($formViewType === 'modal')
        @include('produccion/abms/articulos.create-modal', ['registro' => []])
    @endif
    @if ($formViewType === 'modal')
    @foreach ($registros as $registro)
        @include('produccion/abms/articulos.edit-modal', ['registro' => $registro])
    @endforeach
    @endif
