...
<div class="container-fluid px-0">
    ...
    <form action="{{ route('produccion.abms.articulos.marcas.index') }}" method="GET" class="mb-3 d-flex flex-wrap gap-2">
        <div class="input-group">
            <input type="text" name="buscar" value="{{ request('buscar') }}" class="form-control" placeholder="Buscar...">
            <button type="submit" class="btn btn-outline-primary">Buscar</button>
            @if(request('buscar'))
                <a href="{{ route('produccion.abms.articulos.marcas.index') }}" class="btn btn-outline-secondary">Limpiar</a>
            @endif
        </div>
    ...
    {{-- ➕ Crear nuevo --}}
    <a href="{{ route('produccion.abms.articulos.marcas.create') }}" class="btn btn-success mb-3">➕ Nuevo</a>
        ...
        <table class="table table-striped table-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    {{-- Acciones --}}
                    <td class="text-end">
                        <a href="{{ route('produccion.abms.articulos.marcas.edit', $registro[$primaryKey]) }}" class="btn btn-sm btn-primary">✏️</a>

                        <form action="{{ route('produccion.abms.articulos.marcas.destroy', $registro[$primaryKey]) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar este registro?')">
                            @csrf
                            @method('DELETE')
