    ...
    <div class="modal fade show" id="modalEdit" tabindex="-1" aria-labelledby="modalEditLabel" style="display: block;" aria-modal="true" role="dialog">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{ route('produccion.abms.pasos_rutas_produccion.update', $registro->{$primaryKey}) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalEditLabel">Editar {{ $modelo }}</h5>
                        <a href="{{ route('produccion.abms.pasos_rutas_produccion.index') }}" class="btn-close"></a>
                    </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <a href="{{ route('produccion.abms.pasos_rutas_produccion.index') }}" class="btn btn-secondary">❌ Cancelar</a>
                    </div>
                </form>
            </div>
        </div>
