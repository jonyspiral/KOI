...
<div class="container-fluid px-0">
    <h2 class="mb-4">Listado de User</h2>

    {{-- 🔍 Formulario de búsqueda y cantidad por página --}}
    <form action="{{ route('sistemas.abms.user.index') }}" method="GET" class="mb-3 d-flex flex-wrap gap-2">
        <div class="input-group">
            <input type="text" name="buscar" value="{{ request('buscar') }}" class="form-control" placeholder="Buscar...">
            <button type="submit" class="btn btn-outline-primary">Buscar</button>
            @if(request('buscar'))
                <a href="{{ route('sistemas.abms.user.index') }}" class="btn btn-outline-secondary">Limpiar</a>
            @endif
        </div>
...
@if ($formViewType === 'modal')
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            @include('sistemas/users.create')
        </div>
    </div>
</div>
@else
    <a href="{{ route('sistemas.abms.user.create') }}" class="btn btn-success mb-3">➕ Nuevo</a>
@endif
        ...
        <table class="table table-striped table-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    {{-- Acciones --}}
                    <td class="text-end">
                        <a href="{{ route('sistemas.abms.user.edit', $registro[$primaryKey]) }}" class="btn btn-sm btn-primary">✏️</a>

                        <form action="{{ route('sistemas.abms.user.destroy', $registro[$primaryKey]) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar este registro?')">
                            @csrf
                            @method('DELETE')
